...
export default function ImageWatermark() {
  ...
  const download = () => {
    ...
    render(canvas)
    canvas.toBlob((blob) => { downloadBlob('watermarked.png', blob); setBusy(false) }, 'image/png')
  }
    ...
    <div className="tool-body">
      ...
      {img && (
        <>
          ...
          <div className="row" style={{ marginTop: 14 }}>
            <button className="btn btn-primary" disabled={busy} onClick={download}>{busy ? <span className="spinner" /> : 'Download watermarked image'}</button>
          </div>
        </>
      )}
    </div>
