...
export default function ImageFormatConverter() {
  ...
  const download = () => {
    ...
    canvas.toBlob((blob) => { downloadBlob(`converted.${target.ext}`, blob); setBusy(false) }, target.mime, 0.92)
  }
    ...
    <div className="tool-body">
      ...
      {img && (
        <>
          ...
          <div className="row" style={{ marginTop: 14 }}>
            <button className="btn btn-primary" disabled={busy} onClick={download}>{busy ? <span className="spinner" /> : `Download as ${target.label}`}</button>
          </div>
        </>
      )}
    </div>
