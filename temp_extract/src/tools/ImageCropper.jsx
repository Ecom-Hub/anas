...
export default function ImageCropper() {
  ...
  const download = () => {
    ...
    canvas.toBlob((blob) => { downloadBlob('cropped.png', blob); setBusy(false) }, 'image/png')
  }
      ...
      {src && (
        <div style={{ marginTop: 14 }}>
          ...
          <div className="row" style={{ marginTop: 14 }}>
            <button className="btn btn-primary" disabled={busy} onClick={download}>{busy ? <span className="spinner" /> : 'Download cropped image'}</button>
          </div>
        </div>
      )}
    </div>
