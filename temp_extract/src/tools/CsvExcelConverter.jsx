...
export default function CsvExcelConverter() {
  ...
  const onFile = async (file) => {
    setError(''); setStatus(''); setResult(null); setBusy(true)
  ...
  return (
    <div className="tool-body">
      ...
      <div className="dropzone" style={{ marginTop: 16 }} onClick={() => document.getElementById('csvxlsx-input').click()}>
        <p>{busy ? <span className="spinner" style={{ borderTopColor: 'var(--accent)', borderColor: 'var(--line)' }} /> : (fileName || `Click to choose a ${mode === 'csv2xlsx' ? '.csv' : '.xlsx / .xls'} file`)}</p>
      </div>
      <input
        id="csvxlsx-input"
        type="file"
